...
-- | Substitute a Boolean for a variable and then simplify the formula.
...
-- A clause is a formula that is a disjunction of positive or negative form of variables,
-- such as x1 \/ x2 \/ ~x3
data Clause
  = Clause
      {-# UNPACK #-} !Int -- cached size
      !(IM.IntMap Bool)
...
-- | Join list
data Join a = Empty | NonEmpty (JoinNonEmpty a) -- Intentionally lazy
  deriving (Functor, Foldable, Traversable)

-- | Non-empty join list
data JoinNonEmpty a = Single a | Join (JoinNonEmpty a) (JoinNonEmpty a) -- Intentionally lazy
  deriving (Functor, Foldable, Traversable)
...
{-# INLINE singletonQ #-}
singletonQ :: (a -> Int) -> a -> MyQueue a
singletonQ size n
  | size n == 1 = MyQueue (NonEmpty $ Single n) Empty
...
-- | Pick an element from a join-list.
...
-- | pickOne extracts an element from MyQueue where size-1 elements have priority.
...
-- | Convert a formula into a CNF, while keeping the correspondence between names.
...
-- | We know v is b. So, 'assign' simplify the CNF with this information.
  ...
  return $
    foldr
      ( \c r ->
          case tryAssign c of
            Nothing -> r
            Just c' -> singletonQ sizeC c' `mergeQ` r
      )
      emptyQ
      cs
  where
    {-# INLINEABLE tryAssign #-}
    tryAssign :: Clause -> Maybe Clause
    tryAssign m =
      case lookupC v m of
        Just b' ->
          if b == b' -- the assignment make the literal true
            then
...
-- | This implements unit propagation. That is, if we find a singleton clause like x1 /\ .../~x1 /\ ...
-- we know that x1 must be true and thus propagate the information.
  ...
  Just (x, q') ->
    case toListC x of
      [(v, b)] -> do
        q'' <- assign v b q'
        unitProp q''
      _ -> cannotHappen "size 1 clause is a singleton"
...
dpll cs
  ...
  | otherwise = do
      ...
      case pickOneVariable cs' of
        Nothing -> dpll cs' -- this case, the leftmost clause is empty and thus the call must return mzero
        Just v ->
          (dpll =<< assign v True cs') `mplus` (dpll =<< assign v False cs')

satI :: CNFimpl -> Maybe [(Int, Bool)]
satI cs =
  -- use list monad as a MonadPlus instance
