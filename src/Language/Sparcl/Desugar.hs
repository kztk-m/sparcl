...
type Desugar a = ReaderT NameSource TC a

-- type MonadDesugar m = (MonadReader Int m, MonadTypeCheck m)

withNewName :: (Name -> Desugar r) -> Desugar r
withNewName k = do
  i <- ask
  local (+ 1) (k (Generated i Desugaring))

withNewNames :: Int -> ([Name] -> Desugar r) -> Desugar r
withNewNames n k = do
  i <- ask
  local (+ n) (k $ map (flip Generated Desugaring) [i .. i + n - 1])

runDesugar :: Desugar a -> TC a
...
desugarExp :: S.LExp 'TypeCheck -> Desugar (C.Exp Name)
desugarExp (Loc _ expr) = go expr
  where
    go :: S.Exp 'TypeCheck -> Desugar (C.Exp Name)
...
desugarRHS :: [([S.LPat 'TypeCheck], S.Clause 'TypeCheck)] -> Desugar (C.Exp Name)
...
convertClauseU :: S.Clause 'TypeCheck -> Desugar (C.Exp Name)
convertClauseU (S.Clause body ws Nothing) =
  desugarExp (noLoc $ S.Let ws body)
convertClauseU _ = error "Cannot happen."

convertClauseR :: S.Clause 'TypeCheck -> Desugar (C.Exp Name, C.Exp Name)
...
desugarTopDecls :: S.Decls 'TypeCheck (S.LDecl 'TypeCheck) -> Desugar (C.Bind Name)
