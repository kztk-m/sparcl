...
{-# SPECIALIZE bench :: String -> Int -> TC a -> TC a #-}
...
checkType fp trials = runM $ do
  ...
  let nameTable = mcNameTable (miModuleContext baseModuleInfo)
  let opTable = mcOpTable (miModuleContext baseModuleInfo)
  ...
  let tyEnv = mcTypeTable (miModuleContext baseModuleInfo)
  let conEnv = mcConTable (miModuleContext baseModuleInfo)
  let synEnv = mcSynTable (miModuleContext baseModuleInfo)
